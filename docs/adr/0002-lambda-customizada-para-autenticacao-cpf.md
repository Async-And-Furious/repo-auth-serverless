...
- Já temos acesso e familiaridade com AWS Lambda via a conta de aluno (ADR-0001), reduzindo curva de aprendizado frente a configurar User Pools do Cognito.

**Trade-offs**
- Assumimos 100% da responsabilidade pela segurança do fluxo de autenticação (validação de CPF, proteção contra brute-force, rotação de segredo do JWT, expiração) — sem os recursos prontos do Cognito (MFA, hosted UI, etc.).
